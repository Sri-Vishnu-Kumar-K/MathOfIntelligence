...
def compute_total_error(b, m, points, reg_type = None, tuning_rate = None): #Computes total mean squared error
    totalError = 0
...
def step_gradient(curr_m, curr_b, points, lr, reg_type=None, tuning_rate=None): #Gradient Descent Function.
    ...
    dt_dm = 0 #Initialize the partial derivative w.r.t m_0
    for i in range(len(points)):
        x = points[i,0]
        y = points[i,1]
...
def gradient_descent_driver(points, start_m, start_b, lr, num_iterations, early_stop_number = 0, modify_lr = False, reg_type=None, tuning_rate=None):# The gradient_descent driver function. Controls the number of iterations and also other descent features like early stopping and variable learning rate.
    ...
    for i in range(num_iterations):
        m, b = step_gradient(m, b, points, lr, reg_type, tuning_rate) #Get the new m and b vals.
        er = compute_total_error(b,m,points, reg_type, tuning_rate) #Compute Mean Square error for new m,b
        ...
        if modify_lr and er<compute_total_error(b,m,points, reg_type, tuning_rate): #For modified lr
...
def main(): #main driver function
    ...
    for type_i in [None, 'l1', 'l2']:
        print("Starting gradient descent at b = {0}, m = {1}, error = {2}".format(initial_b, initial_m, compute_total_error(initial_b, initial_m, points, reg_type=type_i, tuning_rate=tuning_rate)))
        print("Running...")
        [m, b, error] = gradient_descent_driver(points, initial_m, initial_b, learning_rate, num_iterations, early_stop_number = 5, modify_lr = True, reg_type=type_i, tuning_rate=tuning_rate)
